...
getup ALL=(ALL) NOPASSWD: ALL (Foi adicionado esse linha para dar permissão sudo para todos os comandos, sem solicitação de senha ) 
...
Para identificar o serviço e qual era o erro 
...
Acessei esse arquivo porque é nele que estão as opções de inicialização do do serviço e na linha do ExecStart=/usr/sbin/nginx tinha um -BROKEN e após conferir no manual pude checar que essa opcao nao existe para iniciar o nginx
